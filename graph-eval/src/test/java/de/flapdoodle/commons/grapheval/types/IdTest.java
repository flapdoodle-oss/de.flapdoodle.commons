...
class IdTest {
	...
	void idAsString() {
		try(Id.ClearTypeCounter __ = Id.with(new TypeCounter())) {
			Id<String> stringId = Id.idFor(TypeInfo.of(String.class));
	...
	void idForTypeMustUseTypeCounter() {
		...
		try(Id.ClearTypeCounter __ = Id.with(localTypeCounter)) {
			Id<String> stringId = Id.idFor(TypeInfo.of(String.class));
			assertThat(stringId.type()).isEqualTo(TypeInfo.of(String.class));
			...
			Id<Integer> intId = Id.idFor(TypeInfo.of(Integer.class));
			assertThat(intId.type()).isEqualTo(TypeInfo.of(Integer.class));
			assertThat(intId.count()).isEqualTo(0);
			assertThat(intId.asInstance("foo")).isEmpty();
			assertThat(intId.asInstance(1)).contains(1);
		}

		assertThat(localTypeCounter.count(TypeInfo.of(String.class))).isEqualTo(1);
		assertThat(localTypeCounter.count(TypeInfo.of(Integer.class))).isEqualTo(1);
	}
	...
	void notSameIdConcurrentAccess() throws InterruptedException {
		...
		for (int i=0;i<numberOfThreads;i++) {
			Thread thread = new Thread(() -> collectedIds.add(Id.idFor(TypeInfo.of(String.class))));
			thread.start();
			threads.add(thread);
		}
