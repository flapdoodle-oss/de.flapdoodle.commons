...
class RelatedTo2Test {
	...
	void checkAttributes() {
		...
		RelatedTo2<Integer, Integer, Integer> testee = RelatedTo2.with(destination, valueA, valueB, (it, a, b) -> Validation.error("error", it.get(), a.value(), b.value()));
		...
		List<ErrorMessage> result = testee.validate(Optional.of(1), StrictValidatedValueLookup.with(
			Arrays.asList(ValidatedValue.of(valueA, 2), ValidatedValue.of(valueB, 3))
		));

		assertThat(result).containsExactly(ErrorMessage.of("error", 1, 2, 3));
	}
}