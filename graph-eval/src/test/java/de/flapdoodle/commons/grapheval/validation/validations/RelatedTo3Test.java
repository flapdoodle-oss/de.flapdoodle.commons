...
import de.flapdoodle.commons.grapheval.validation.ValidatedValue;
...
class RelatedTo3Test {
	...
	void checkAttributes() {
		...
		RelatedTo3<Integer, Integer, Integer, Integer> testee = RelatedTo3.with(destination, valueA, valueB, valueC, (it, a, b, c) -> Validation.error("error", it.get(), a.value(), b.value(), c.value()));
		...
		List<ErrorMessage> result = testee.validate(Optional.of(1), StrictValidatedValueLookup.with(
			Arrays.asList(ValidatedValue.of(valueA, 2), ValidatedValue.of(valueB, 3), ValidatedValue.of(valueC, 4))
		));

		assertThat(result).containsExactly(ErrorMessage.of("error", 1, 2, 3, 4));
	}
}