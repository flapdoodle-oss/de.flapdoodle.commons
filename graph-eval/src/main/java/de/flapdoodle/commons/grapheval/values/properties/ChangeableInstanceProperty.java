...
public abstract class ChangeableInstanceProperty<O extends IsChangeableInstance<O, M>, M extends O, T> implements IsChangeableProperty<O, T> {
    ...
    @Value.Parameter
    protected abstract BiFunction<M, T, O> copyOnWrite();
    ...
    public O change(O instance, T value) {
        Preconditions.checkArgument(type().isInstance(instance), "instance type mismatch: %s != %s", type(), instance);
        return instance.change(it -> copyOnWrite().apply(it, value));
    }
    ...
    public static <O extends IsChangeableInstance<O, M>, M extends O, T> ChangeableInstanceProperty<O, M, T> of(
            Class<O> type,
            String name,
            Function<O, T> getter,
            BiFunction<M, T, O> copyOnWrite
    ) {
        return ImmutableChangeableInstanceProperty.of(TypeInfo.of(type), name, getter, copyOnWrite);
    }
    ...
    public static <O extends IsChangeableInstance<O, M>, M extends O, T> ChangeableInstanceProperty<O, M, T> of(
        ...
        return ImmutableChangeableInstanceProperty.of(type, name, getter, copyOnWrite);
    }
}
