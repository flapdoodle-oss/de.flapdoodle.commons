...
import de.flapdoodle.commons.grapheval.calculate.functions.FN5;
import de.flapdoodle.commons.grapheval.calculate.functions.FN6;
...
public abstract class Merge6<A, B, C, D, E, F, X> implements Calculation<X>, HasHumanReadableLabel {
	@Value.Parameter
	...
	@Value.Parameter
	protected abstract ValueSource<F> f();

	@Value.Parameter
	protected abstract FN6<A, B, C, D, E, F, X> transformation();

	@Override
	public Set<ValueSource<?>> sources() {
		return ImmutableSet.of(a(), b(), c(), d(), e(), f());
	}

	@Override
	public X calculate(ValueLookup values) {
		return transformation().apply(values.get(a()), values.get(b()), values.get(c()), values.get(d()), values.get(e()), values.get(f()));
	}
	...
	public static <A, B, C, D, E, F, X> Merge6<A, B, C, D, E, F, X> with(
		ValueSource<A> a,
		ValueSource<B> b,
		ValueSource<C> c,
		ValueSource<D> d,
		...
		FN6<A, B, C, D, E, F, X> transformation
	) {
		return ImmutableMerge6.of(destination, a, b, c, d, e, f, transformation);
	}
}
